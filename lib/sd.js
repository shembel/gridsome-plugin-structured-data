export default function (config, options, context) {
	...
	const modifiedDate = ((ogtype == 'website' && !modified) || modified == 'now') ? new Date().toISOString() : modified
	...
	const metaWebsite = [
		{
			...
			itemref: 'mdUrl mdName mdDescription mdImage mdDatePublished mdDateModified mdAuthor mdPublisher',
			property: 'og:type',
			content: ogtype
		}
	]
	const metaArticle = [
		{
			...
			itemref: 'mdHeadline mdDescription mdImage mdDatePublished mdDateModified mdMainEntityOfPage mdAuthor mdPublisher',
			property: 'og:type',
			content: ogtype
		}
	]
	...
	(ogtype == 'website') && metaArry.push(...metaName);
